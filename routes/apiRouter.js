...
apiRouter.post('/dishes', function(request, response) {
	let dish = new Dish(request.body) //create new instance of schema, request.body is all the information that we have taken from the client side and we send it on the body of the request to the server
...
apiRouter.get('/dishes', function(request, response) {
	...
	// request.querry parses the parameters and turns them into an object
	if (error) {
		response.send(error)
	}
	else {
...
apiRouter.get('/user/dishes', function(request, response){
	Dish.find({authorId: request.user._id}, function(error, records) {///wants to get all dishes where the author id on the dish matches the id on the current user
		if (error) {
		response.send(error)
		}
		else {
